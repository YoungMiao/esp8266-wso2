...
void OfficeFrameHelperWin::initStyleOption(StyleOptionFrame* option)
{
    ...
    option->tabBarRect = RibbonBarPrivate::_get(m_ribbonBar)->m_tabBar->geometry();
}
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
    Q_UNUSED(painter);
    hdc = backingStoreDC(m_frame);
#else
    QPaintEngine* en = painter->paintEngine();
	hdc = en != Q_NULL ? en->getDC() : hdc;
#endif
...
void OfficeFrameHelperWin::frameChanged()
{
    if (m_hwndFrame)
        ::SetWindowPos(m_hwndFrame, NULL, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_NOZORDER | SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
}
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
bool OfficeFrameHelperWin::collapseTopFrame()
{
    ...
    if (QWindow* window = m_frame->windowHandle()) 
    {
        if (QPlatformWindow* platformWindow = window->handle()) 
        {
            QVariant oldVariant = QGuiApplication::platformNativeInterface()->windowProperty(platformWindow, QStringLiteral("WindowsCustomMargins"));
            if (!oldVariant.isValid())
                return false;

            int val = 0;
            int valBottom = 0;
            bool visible = theme2013 ? true : RibbonBarPrivate::_get(m_ribbonBar)->m_ribbonBarVisible;
            ...
            QMargins oldcustomMargins = qvariant_cast<QMargins>(oldVariant);
...
bool OfficeFrameHelperWin::winEvent(MSG* msg, long* result)
{
    ...
    else if (m_hwndFrame != Q_NULL && msg->message == WM_NCHITTEST  && !isTitleVisible() && m_dwmEnabled)
    {
        ...
        if ( *result == HTCLIENT || *result == HTSYSMENU)
        {
            ...
            rc.bottom = rc.top + RibbonBarPrivate::_get(m_ribbonBar)->m_tabBar->geometry().top() * OfficeFrameHelperWin::m_devicePixelRatio;
    ...
    else if (m_hwndFrame != Q_NULL && msg->message == WM_SHOWWINDOW)
    {
        ...
        if (!bShow)
        {
            ...
            bool visible = is2013 ? true : RibbonBarPrivate::_get(m_ribbonBar)->m_ribbonBarVisible;
