...
{
	QTN_D(const RibbonBackstageButton);
	RibbonBackstageButton* thisButton = (RibbonBackstageButton*)this;
    ...
    if (opt.toolButtonStyle != Qt::ToolButtonIconOnly)
    {
        if (opt.toolButtonStyle == Qt::ToolButtonTextUnderIcon)
        {
            if (!opt.icon.isNull())
            {
                QSize actualSize = opt.icon.actualSize(opt.iconSize, QIcon::Normal, QIcon::Off);
                const int ret = style()->property("DPIAware").toBool() ? int(DrawHelpers::dpiScaled(actualSize.width())) : actualSize.width();
                sz = QSize(ret, ret);
            }
			else
			{
            ...
            sz.setWidth(sz.height() + opt.fontMetrics.width('X'));
            return sz;
        }
...
bool RibbonBackstageButton::event(QEvent* event)
{
    ...
    if (type == QEvent::StyleChange || type == QEvent::FontChange || type == QEvent::Show || type == QEvent::Resize)
    {
...
static void rib_QTextDocument_draw(QPainter& painter, QTextDocument* document, const QPalette& palette, const QRect& rect)
{
    painter.save();
...
void RibbonBackstageButton::paintEvent(QPaintEvent* event)
{
    ...
    if (option.toolButtonStyle == Qt::ToolButtonTextUnderIcon)
    {
        ...
        {
            QSize actualSize = option.icon.actualSize(option.iconSize, QIcon::Normal, QIcon::Off);
            int ret = style()->property("DPIAware").toBool() ? int(DrawHelpers::dpiScaled(actualSize.width())) : actualSize.width();
		...
		option.toolButtonStyle == Qt::ToolButtonTextOnly)
    {
		...
		const int indentation = option.icon.isNull() ? (option.fontMetrics.height() / 2 + 2) / 2 : 
            option.iconSize.height() + (option.fontMetrics.height() / 2 + 2) / 2;
		rcText.adjust(0, indentation, 0, 0);
		rcText.setHeight(option.rect.height() - indentation);

        option.palette.setColor(QPalette::Text, option.palette.color(QPalette::ButtonText));
		d.m_document->setTextWidth(static_cast<qreal>(rcText.width()));
		QSize szDoc = d.m_document->size().toSize();
		QRect rcTxt = rcText;
//        if (d.rowCount() != 1)
        {
		...
		::rib_QTextDocument_draw(p, d.m_document, option.palette, rcTxt);

		if (option.features & QStyleOptionToolButton::HasMenu)
		{
			QTextBlock block = d.m_document->firstBlock();
			if (block.isValid())
			{
				...
				if (count > 0)
				{
					...
					if (tline.isValid())
					{
						...
						rectRow.translate((option.rect.width() - minWidth) / 2, 0);
                        rectRow.adjust(0, option.fontMetrics.height()*(count - 1), 0, 0);
						rectRow.setWidth(minWidth);
                        const int retArrow = 5;//style()->property("DPIAware").toBool() ? int(DrawHelpers::dpiScaled(5)) : 5;
						if (count == 1)
							option.rect = QRect(QPoint(rectRow.center().x() - retArrow / 2,
                                rectRow.center().y() + szDoc.height()/2 - retArrow/2), QSize(retArrow, retArrow));
						else
							option.rect = QRect(QPoint(rectRow.right() + option.fontMetrics.width('x') - retArrow / 2,
