...
RibbonGroupPrivate* RibbonGroupPrivate::_get(RibbonGroup* group)
{
    return &group->qtn_d();
}
...
void RibbonGroupPrivate::updateOptionButtonLayout()
{
    ...
    if (m_optionButton && m_butOptionVisible)
    {
        ...
        initStyleOption(opt);
        QRect rc = opt.rect;
        rc.setTop(rc.bottom() - opt.lineWidth);
        QSize sz(opt.lineWidth - 3, opt.lineWidth - 3);
        QRect rectButOption(QPoint(rc.right() - sz.width() - 2, rc.top() + (rc.height() - sz.height())/2), sz);
