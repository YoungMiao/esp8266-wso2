...
QSize RibbonToolTip::sizeHint() const
{
    ...
    if (drawImage)
    {
        ...
        if (QToolButton* button = qobject_cast<QToolButton*>(d.m_widget))
        {
            ...
            const int sz = style()->pixelMetric(drawImageTop ? QStyle::PM_SmallIconSize : QStyle::PM_LargeIconSize, 0, button);
            szIcon = QSize(sz, sz);
        }
        else
        {
    ...
    if (drawTitle)
    {
        const int siz = style()->pixelMetric(QStyle::PM_SmallIconSize, 0, this);
...
void RibbonToolTip::paintEvent(QPaintEvent* event)
{
    ...
    if (drawImage)
    {
        if (QToolButton* button = qobject_cast<QToolButton*>(d.m_widget))
        {
            drawImageTop = button->toolButtonStyle() != Qt::ToolButtonTextUnderIcon;
            const int sz = style()->pixelMetric(drawImageTop ? QStyle::PM_SmallIconSize : QStyle::PM_LargeIconSize, 0, button);
            qreal dpi = style()->property("DPIAware").toBool() ? static_cast<qreal>(DrawHelpers::getDPIToPercent())/100.0 : 1.0;
        ...
        if (drawImageTop)
        {
            ...
            QPixmap pm = d.m_icon.pixmap(szDrvImage, QIcon::Normal, QIcon::On);
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
            QSize sz = pm.size() / pm.devicePixelRatio();
#else
            QSize sz = pm.size();
#endif
            if (sz.width() * sz.height() < szDrvImage.width() * szDrvImage.height())
                pm = pm.scaled(szDrvImage, Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
bool RibbonToolTip::event(QEvent* event)
{
    QTN_D(RibbonToolTip);
