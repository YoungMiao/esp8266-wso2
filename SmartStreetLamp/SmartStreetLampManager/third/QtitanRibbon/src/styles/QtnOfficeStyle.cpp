...
QString OfficeStylePrivate::theme(const QString& str) const
{
    ...
    switch (m_themeType) 
    {
        ...
        case OfficeStyle::Office2013Dark:
            ...
            break;
        case OfficeStyle::Office2016Colorful:
            strStyle += ":/res/Office2016Colorful/";
            break;
        case OfficeStyle::Office2016White:
            strStyle += ":/res/Office2016White/";
            break;
        case OfficeStyle::Office2016DarkGray:
            strStyle += ":/res/Office2016DarkGray/";
            break;
        case OfficeStyle::Office2016Black:
            strStyle += ":/res/Office2016Black/";
            break;
        default:
            Q_ASSERT(false);
            break;
...
OfficePaintManagerInterface* OfficeStylePrivate::officePaintManager() const
{
    ...
    OfficePaintManagerInterface* officePaintManager = qobject_cast<OfficePaintManagerInterface*>(p.paintManager());
...
{
    QTN_P(OfficeStyle)
    OfficeStyle::Theme theme = p.getTheme();
    ...
    else if (theme == OfficeStyle::Office2016Colorful || theme == OfficeStyle::Office2016White || theme == OfficeStyle::Office2016DarkGray || theme == OfficeStyle::Office2016Black)
        setPaintManager(*new OfficePaintManager2016(&p));
    else
...
/*! \internal */
bool OfficeStylePrivate::isScrollBarsIgnored() const
{
...
Sets the animation for Microsoft Office's controls when mouse over the elements to \a enable state. By default the animation is set.
 */
...
bool OfficeStyle::isDPIAware() const
{
    ...
    return d.m_dpiAware && (!QCoreApplication::testAttribute(Qt::AA_EnableHighDpiScaling) && !qEnvironmentVariableIsSet("QT_AUTO_SCREEN_SCALE_FACTOR"));
#else
    return d.m_dpiAware;
#endif
}
...
/*!
Returns the value of the DPI setting in native format. If isDPIAware() is false then this function always returns 96 for WIndows and Linux and 72 for MacOSX.
*/
...
{
    return widget && ::topLevelParentWidget(widget) != Q_NULL;
}
...
int OfficeStyle::pixelMetric(PixelMetric pm, const QStyleOption* opt, const QWidget* w) const
{
    ...
    switch(pm)
    {
        case PM_IndicatorWidth:
                ret = isDPIAware() ? int(DrawHelpers::dpiScaled(13.)) : 13;
            break;
        case PM_IndicatorHeight:
                ret = isDPIAware() ? int(DrawHelpers::dpiScaled(13.)) : 13;
            break;
        case PM_ExclusiveIndicatorWidth:
                ret = isDPIAware() ? int(DrawHelpers::dpiScaled(12.)) : 12;
            break;
        case PM_ExclusiveIndicatorHeight:
                ret = isDPIAware() ? int(DrawHelpers::dpiScaled(12.)) : 12;
            break;
        case QStyle::PM_ScrollBarExtent:
                if (w && w->inherits("Qtitan::RibbonGallery"))
                    ret = isDPIAware() ? int(DrawHelpers::dpiScaled(14.)) : 14;
                else
        ...
        case PM_ButtonIconSize:
            {
                if (w && (w->inherits("Qtitan::RibbonSystemPopupBar") || w->inherits("Qtitan::RibbonPageSystemPopup")))
                    ret = isDPIAware() ? int(DrawHelpers::dpiScaled(32.)) : 32;
                else
                    ret = isDPIAware() ? int(DrawHelpers::dpiScaled(16.)) : 16;
            }
            break;
...
void OfficeStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    ...
    bool ok = false;
    if (getTheme() == OfficeStyle::Office2016Black)
        ok = element == QStyle::PE_IndicatorBranch && qobject_cast<const QTreeView*>(widget);
...
/*! \internal */
QIcon OfficeStyle::modifyIcon(QStyle::StandardPixmap px, const QStyleOption* opt, const QWidget* wd, bool& ret) const 
{ 
    ret = false;
#if defined(Q_OS_WIN)
    if (QStyle* style = defaultStyle())
    {
        if (getTheme() == OfficeStyle::Office2016Black && wd && qobject_cast<const QDockWidget*>(wd) &&
            (px == QStyle::SP_TitleBarNormalButton || px == QStyle::SP_TitleBarCloseButton))
        {
            ...
            {
                ret = true;
                int iconSize = style->proxy()->pixelMetric(QStyle::PM_SmallIconSize, 0, wd);
...
// for QForm
/*! \internal */
bool OfficeStyle::drawFrameTabWidget(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    QTN_D(const OfficeStyle);
    return d.officePaintManager()->drawFrameTabWidget(opt, p, w);
}

/*! \internal */
bool OfficeStyle::drawFrameTabBarBase(const QStyleOption* opt, QPainter* p, const QWidget* w) const 
{ 
    QTN_D(const OfficeStyle);
    return d.officePaintManager()->drawFrameTabBarBase(opt, p, w);
}
...
/*! \internal */
bool OfficeStyle::drawFrame(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    QTN_D(const OfficeStyle);
    return d.officePaintManager()->drawFrame(opt, p, w);
}
...
bool OfficeStyle::drawMenuBarItem(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    if (const QStyleOptionMenuItem* mbi = qstyleoption_cast<const QStyleOptionMenuItem*>(opt))
    {
        ...
        if (!pix.isNull())
            QStyle::drawItemPixmap(p, mbi->rect, alignment, pix);
        else
...
bool OfficeStyle::drawMenuItem(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionMenuItem* menuitem = qstyleoption_cast<const QStyleOptionMenuItem*>(opt)) 
    {
        ...
        bool activated = menuitem->state & State_Selected;

        // windows always has a check column, regardless whether we have an icon or not
        int iconSize = proxy()->pixelMetric(PM_ToolBarIconSize, opt, widget);
...
#endif /* Q_OS_LINUX */
        if (activated)
            drawRectangle(p, menuitem->rect, true/*selected*/, false/*pressed*/, !disabled/*enabled*/,
        ...
        {
            iconSize = proxy()->pixelMetric(PM_LargeIconSize, opt, widget);
            if (iconSize > opt->rect.height())
                iconSize = proxy()->pixelMetric(PM_SmallIconSize, opt, widget);
            checkcol = iconSize + 6;
        }
        ...
        if (!menuitem->icon.isNull()) 
        {
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
            QPixmap pixmap = checked ? menuitem->icon.pixmap(iconSize, mode, QIcon::On) : menuitem->icon.pixmap(iconSize, mode);
            ...
            {
                pmr.setX(6 + 4 + 4);
                pmr.setY(menuitem->rect.y() + (menuitem->rect.height() - iconSize) / 2);
            }
            else
            ...
            if (checked)
            {
                ...
                drawRectangle(p, rcChecked.adjusted(-2, 0, 2, 0), false/*selected*/, true/*pressed*/, !disabled/*enabled*/,
...
bool OfficeStyle::drawToolButtonLabel(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (w && !qobject_cast<const RibbonButton*>(w) && !qobject_cast<const PopupColorButton*>(w))
    {
        QWidget* parent = w->parentWidget();
        if (parent && !qobject_cast<const RibbonQuickAccessBar*>(parent) && !qobject_cast<const QStatusBar*>(parent))
            return false;
    }
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        else 
        {
            ...
            if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly)
            {
                ...
                QRect popupr = proxy()->subControlRect(QStyle::CC_ToolButton, toolbutton, QStyle::SC_ToolButtonMenu, w);
                ...
                if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
                {
                    ...
                    proxy()->drawItemText(p, rcText, alignment, toolbutton->palette,
...
bool OfficeStyle::drawToolButton(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        if (toolbutton->subControls & SC_ToolButtonMenu) 
        {
            ...
            {
                int heightLine = 0;
                int mbi = proxy()->pixelMetric(PM_MenuButtonIndicator, toolbutton, widget) / 2 + 2;
...
/*! \internal */
bool OfficeStyle::drawIndicatorViewItemCheck(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const 
{ 
    QTN_D(const OfficeStyle);
...
bool OfficeStyle::drawDockWidgetTitle(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionDockWidget* dwOpt = qstyleoption_cast<const QStyleOptionDockWidget*>(opt)) 
    {
        ...
        if (!dwOpt->title.isEmpty()) 
        {
            ...
            const int indent = 4;
            proxy()->drawItemText(p, rect.adjusted(indent + 1, 1, -indent - 1, -1), Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, palette,
...
// for ComboBox
/*! \internal */
bool OfficeStyle::drawComboBoxLabel(ControlElement element, const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    bool ret = false;
    if (getTheme() == OfficeStyle::Office2016Black)
        return ret;
...
{
    bool drawDefaultStyle = !(getTheme() == OfficeStyle::Office2016Black && (qobject_cast<const QListView*>(widget) || qobject_cast<const QTreeView*>(widget)));
