        ...
        if (d.m_flatFrame && optRibbonBar->frameHelper)
        {
            if (!optRibbonBar->frameHelper->isDwmEnabled())
            {
                if (!optRibbonBar->frameHelper->isActive())
                    optRibbonBar->frameHelper->fillSolidRect(p, rcRibbonTabs, d.m_clrRibbonInactiveFace);
            }
            else
            {
...
void RibbonPaintManager::drawReducedGroup(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionToolButton* optGroup = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        if (!optGroup->icon.isNull())
        {
            ...
            pmSize = pm.size();
#endif
            if (pmSize.width() * pmSize.height() < optGroup->iconSize.width() * optGroup->iconSize.height())
                pm = pm.scaled(optGroup->iconSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);

            soImage = cached("RibbonGroupButtonIcon.png");
            QSize sz = soImage.size();
            if (baseStyle()->property("DPIAware").toBool())
                sz = QSize( int(DrawHelpers::dpiScaled((qreal)sz.height())), int(DrawHelpers::dpiScaled((qreal)sz.width())) );
            else
            ...
            int indent = baseStyle()->property("DPIAware").toBool() ? static_cast<int>(DrawHelpers::dpiScaled(6.)) : 6;
            rectImage.adjust(-indent, -indent, indent, indent);
            baseStyle()->drawItemPixmap(p, rectImage, Qt::AlignCenter, pm);
        }
        ...
        const int menuArrowWidth = twoLines ? baseStyle()->proxy()->pixelMetric(QStyle::PM_MenuButtonIndicator, optGroup, widget) : 0;
        if (menuArrowWidth > 0)
            strText += QString("   ");

        baseStyle()->proxy()->drawItemText(p, rcText, flags | Qt::TextWordWrap | Qt::AlignHCenter, optGroup->palette,
        ...
        QPoint pt = QPoint(!twoLines ? rcText.center().x() - 4 : rcText.left() + sz.width() + 1,
            !twoLines ? (rcText.bottom() + optGroup->fontMetrics.height()) - (optGroup->fontMetrics.height() - optGroup->fontMetrics.descent()) :
            rcText.bottom() - (optGroup->fontMetrics.height() - optGroup->fontMetrics.descent()));

        QRect rc(pt, rcSrc.size());
        if (baseStyle()->property("DPIAware").toBool())
        {
            rc.setWidth(int(DrawHelpers::dpiScaled(qreal(rc.width()))));
            rc.setHeight(int(DrawHelpers::dpiScaled(qreal(rc.height()))));
            p->save();
            p->setRenderHint(QPainter::SmoothPixmapTransform, true);
...
bool RibbonPaintManager::drawSlider(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSlider* slider = qstyleoption_cast<const QStyleOptionSlider*>(opt))
    {
        ...
        {
            const QRect groove = baseStyle()->proxy()->subControlRect(QStyle::CC_Slider, opt, QStyle::SC_SliderGroove, w);
            const QRect handle = baseStyle()->proxy()->subControlRect(QStyle::CC_Slider, opt, QStyle::SC_SliderHandle, w);
...
void RibbonPaintManager::drawGroupScrollButton(const QStyleOption* option, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(option)) 
    {
        ...
        const QRect rc = toolbutton->rect;
    ...
    if (const StyleSystemToolButton* toolbutton = qstyleoption_cast<const StyleSystemToolButton*>(option))
    {
        if (qobject_cast<const RibbonBar*>(widget->parentWidget()))
        {
            if (toolbutton->toolButtonStyle != Qt::ToolButtonFollowStyle)
            {
                ...
                const bool popuped  = (toolbutton->activeSubControls & QStyle::SC_ToolButtonMenu) && (option->state & QStyle::State_Sunken);
                const bool isHighlighted = option->state & QStyle::State_MouseOver;
        ...
        const bool selected = toolbutton->state & QStyle::State_MouseOver;
        ...
        p->save();
        p->translate(toolbutton->rect.topLeft());
        QPoint ptDest;
        p->setRenderHint(QPainter::SmoothPixmapTransform, true);
        drawImage(soImage, *p, QRect(ptDest, QSize(toolbutton->rect.size())), rcSrc, QRect(QPoint(0, 0), QPoint(0, 0)));
...
void RibbonPaintManager::drawOptionButton(const QStyleOption* option, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionToolButton* opt = qstyleoption_cast<const QStyleOptionToolButton*>(option))
    {
        ...
        rect.setSize(opt->iconSize);
        drawImage(soRibbonOptionButton, *p, rect, rcSrc, QRect(QPoint(3, 2), QPoint(2, 2)), QColor(0xFF, 0, 0xFF));
    }
}
            ...
            case QStyle::PE_IndicatorArrowDown :
                if (qobject_cast<const QToolButton*>(w))
                {
                    if (!qobject_cast<const QTabBar*>(w->parentWidget()))
                    {
                        ...
                        const QRect rcSrc = sourceRectImage(soIndicator.rect(), !enabled ? 3 : selected ? 1 : 0, 4);
                        const QSize sizeImage = rcSrc.size();

                        const QPoint ptImage(opt->rect.left() + (opt->rect.width() - sizeImage.width()) / 2  + ((opt->rect.width()  - sizeImage.width())  % 2), 
                            opt->rect.top()  + (opt->rect.height() - sizeImage.height()) / 2 + ((opt->rect.height() - sizeImage.height()) % 2));

                        QRect rcEntry(ptImage, rcSrc.size());
                        if (baseStyle()->property("DPIAware").toBool())
                        {
                            rcEntry.setWidth(int(DrawHelpers::dpiScaled(qreal(rcEntry.width()))));
                            rcEntry.setHeight(int(DrawHelpers::dpiScaled(qreal(rcEntry.height()))));
                            p->save();
                            p->setRenderHint(QPainter::SmoothPixmapTransform, true);
...
void RibbonPaintManager::drawRibbonBackstageMenu(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionMenuItem* menuitem = qstyleoption_cast<const QStyleOptionMenuItem*>(opt))
    {
        ...
        if ( dis )
            clrText = menuitem->palette.color(QPalette::Disabled, QPalette::WindowText).light();
        ...
        {
            int iconSize = baseStyle()->proxy()->pixelMetric(QStyle::PM_LargeIconSize, opt, widget);
            if (iconSize > qMin(opt->rect.height(), opt->rect.width()))
                iconSize = baseStyle()->proxy()->pixelMetric(QStyle::PM_SmallIconSize, opt, widget);
            ...
            p->save();
            p->setRenderHint(QPainter::SmoothPixmapTransform, true);
            p->drawPixmap(QRect(pmr, QSize(iconSize, iconSize)), pixmap);
            p->restore();
            iconWidth = iconSize;
        }
...
{ 
    ...
    else if (w && qobject_cast<const RibbonTitleButton*>(w))
    {
        ...
        {
            const bool down = toolbutton->state & QStyle::State_Sunken;
...
void RibbonPaintManager2013::drawOptionButton(const QStyleOption* option, QPainter* p, const QWidget* widget) const
{
    ...
    OfficePaintManager2013::ImageState state = OfficePaintManager2013::Black2;
    const OfficePaintManager2013::ImageIcons index = OfficePaintManager2013::Icon_LaunchArrow;
        ...
        switch(pe)
        {
            case QStyle::PE_IndicatorArrowDown :
                if (qobject_cast<const QToolButton*>(w))
                {
                    ...
                    {
                        const bool enabled = opt->state & QStyle::State_Enabled;    
                        const bool highlighted = opt->state & QStyle::State_Selected;
                        const bool pressed = opt->state & QStyle::State_Sunken;

                        const OfficePaintManager2013::ImageIcons index = OfficePaintManager2013::Icon_ArowDown;
                        OfficePaintManager2013::ImageState state = OfficePaintManager2013::Black2;
...
void RibbonPaintManager2013::drawBackstage(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const StyleOptionRibbonBackstage* optBackstage = qstyleoption_cast<const StyleOptionRibbonBackstage*>(opt)) 
    {
        ...
        const int top = (bool)baseStyle()->proxy()->styleHint((QStyle::StyleHint)RibbonStyle::SH_RibbonBackstageHideTabs) ? 0 : 2;
...
//    p->fillRect(opt->rect, backstageCloseButton());
    ...
    if (const QStyleOptionToolButton* optButton = qstyleoption_cast<const QStyleOptionToolButton*>(opt)) 
    {
        if (!optButton->icon.isNull())
        {
            ...
            QPixmap pixmap = optButton->icon.pixmap(optButton->iconSize, enabled ? (highlighted ? QIcon::Selected : QIcon::Active) : QIcon::Disabled, QIcon::On);
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
            const int pixw = pixmap.width() / pixmap.devicePixelRatio();
            const int pixh = pixmap.height() / pixmap.devicePixelRatio();
#else
            ...
            if (pixw * pixh < optButton->iconSize.width() * optButton->iconSize.height())
                pixmap = pixmap.scaled(optButton->iconSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
void RibbonPaintManager2016::drawQuickAccessButton(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    const ImageIcons index = Icon_CustomizeArowDown;

    QRect rectWhite = opt->rect;
    rectWhite.adjust(0, 1, 0, 1);
...
void RibbonPaintManager2016::setupPalette(QWidget* widget) const
{
    ...
    if (themeId == OfficeStyle::Office2016Black)
    {
        ...
        {
            QPalette palette = widget->palette();
            palette.setColor(QPalette::Normal,   QPalette::ButtonText, helper().getColor("ToolButton", "ButtonText"));
            palette.setColor(QPalette::Inactive, QPalette::ButtonText, helper().getColor("ToolButton", "ButtonText"));
            palette.setColor(QPalette::Disabled, QPalette::ButtonText, helper().getColor("ToolButton", "ButtonText").dark(150));
            widget->setPalette(palette);
        }
        else if (qobject_cast<QPushButton*>(widget))
        {
            QPalette palette = widget->palette();
            palette.setColor(QPalette::Normal,   QPalette::ButtonText, Qt::white);
            palette.setColor(QPalette::Inactive, QPalette::ButtonText, helper().getColor("ToolButton", "ButtonText"));
            palette.setColor(QPalette::Disabled, QPalette::ButtonText, helper().getColor("ToolButton", "ButtonText").dark(150));
            widget->setPalette(palette);
        }
        ...
        else if (qobject_cast<QLineEdit*>(widget) || qobject_cast<QComboBox*>(widget))
        {
            ...
            palette.setColor(QPalette::Disabled, QPalette::Text, helper().getColor("ToolButton", "ButtonText").dark(150));
            palette.setColor(QPalette::Base, QColor(67, 67, 67));
            palette.setColor(QPalette::Disabled, QPalette::Shadow, d.m_clrEditCtrlBorder);
            widget->setPalette(palette);
        } 
        else if (qobject_cast<RibbonToolTip*>(widget) || widget->inherits("QTipLabel"))
        {
