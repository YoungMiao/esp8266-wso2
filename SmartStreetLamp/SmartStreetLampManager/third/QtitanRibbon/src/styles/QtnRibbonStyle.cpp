...
static QFont qtn_LOGFONTtoQFont(LOGFONT& lf, bool dpiaware)
{
    ...
    qreal factor = dpiaware ? 1.0 : (qreal)DrawHelpers::defaultDpiY() / g_logPixel;
    ...
    return qFont;
/*
    QString family = QString::fromUtf16(reinterpret_cast<const ushort *>(lf.lfFaceName));
    QFont qf(family);
    ...
    double currentlogPixel = static_cast<double>(GetDeviceCaps(displayDC, LOGPIXELSY));
    double delta = g_logPixel/currentlogPixel;
    double scale = factor*(dpiaware ? delta : 1.0);
...
{
    QTN_P(const RibbonStyle);
    RibbonPaintManagerInterface* ribbonPaintManager = qobject_cast<RibbonPaintManagerInterface *>(p.paintManager());
...
void RibbonStylePrivate::makePaintManager()
{
    ...
    if (theme == OfficeStyle::Office2013White || theme == OfficeStyle::Office2013Gray || theme == OfficeStyle::Office2013Dark)
        ...
        setPaintManager(*new RibbonPaintManager2016(&p));
    else
...
QFont RibbonStyle::font(const QWidget* widget) const
{
    if (qobject_cast<const RibbonBar*>(widget) || widget == Q_NULL)
    {
      ...
      return qtn_LOGFONTtoQFont(ncm.lfMenuFont, isDPIAware());
#else 
      QMenuBar menu;
    ...
    switch(pm) 
    {
        ...
        case PM_MenuBarVMargin:
            {
                ...
                if (!option)
                {
                    if (const RibbonBar* ribbonBar = qobject_cast<const RibbonBar*>(widget))
                        ret += RibbonBarPrivate::_get(ribbonBar)->topBorder() + ribbonBar->titleBarHeight() + 2;
                }
            }
            break;
        ...
        case PM_RibbonReducedGroupWidth : 
            if (const QStyleOptionToolButton* optBut = qstyleoption_cast<const QStyleOptionToolButton*>(option))
            {
                ...
                if (!proxy()->styleHint(QStyle::SH_UnderlineShortcut, optBut, widget))
                    flags |= Qt::TextHideMnemonic;

                QRect rect = optBut->rect;
                QString str = optBut->text.left(optBut->text.indexOf(QLatin1String(" ")));
                int width = optBut->fontMetrics.boundingRect(rect, flags | Qt::AlignVCenter, str).width();
                str = optBut->text.right(optBut->text.indexOf(QLatin1String(" ")));
                width = qMax(width, optBut->fontMetrics.boundingRect(rect, flags | Qt::AlignVCenter, str).width());

                ret = width + optBut->fontMetrics.width(QLatin1String("X")) * 3;
            }
            break;
        case PM_RibbonHeightCaptionGroup :
            ...
            {
                bool titleGroupsVisible = widget ? widget->property(_qtn_TitleGroupsVisible).toBool() : false;
        ...
        case PM_ToolBarSeparatorExtent:
            {
                if (qobject_cast<const RibbonToolBarControl*>(widget))
                    ret = isDPIAware() ? int(DrawHelpers::dpiScaled(3.)) : 3;
...
int RibbonStyle::styleHint(StyleHint hint, const QStyleOption* opt, const QWidget* widget, QStyleHintReturn *returnData) const
{
    ...
    else if (hint == SH_UnderlineShortcut)
    {
        ...
        if (ribbonBar && !qobject_cast<const QMenu*>(widget))
            ret = 0;
        else if (ribbonBar && RibbonBarPrivate::_get(ribbonBar)->m_keyTips.size() > 0)
            ret = 1;
        else
        ...
        case CT_MenuItem :
            {
                ...
                if (menu)
                {
                    if (const QStyleOptionMenuItem* mi = qstyleoption_cast<const QStyleOptionMenuItem*>(opt))
                    {
                        ...
                        {
                            sz.setHeight(qMax(sz.height() - 2, pixelMetric(PM_LargeIconSize, opt, widget)));
                            w -= 6;
                        }
                        ...
                        if (mi->menuItemType != QStyleOptionMenuItem::Separator && !mi->icon.isNull()) 
                        {
                            ...
                            sz.setHeight(qMax(sz.height(), /*mi->icon.actualSize(QSize(iconExtent, iconExtent)).height()*/iconExtent + 2 * windowsItemFrame));
                        }
...
//                    if ((tb->features & (QStyleOptionToolButton::MenuButtonPopup | QStyleOptionToolButton::PopupDelay)) == QStyleOptionToolButton::MenuButtonPopup)
                    {
                        if (!qobject_cast<const RibbonGroupPopupButton *>(widget)) 
                        {
                            if (tb->toolButtonStyle == Qt::ToolButtonTextUnderIcon) 
                            {
                                ...
                                ret.adjust(0, opt->rect.bottom() - textSize.height(), 0, 0);
    ...
    if (const StyleOptionTitleBar* optTitleBar = qstyleoption_cast<const StyleOptionTitleBar*>(opt))
    {
        if (optTitleBar->frameHelper)
        {
            ...
            else
            {
                ...
                if (getTheme() == Office2016Colorful || getTheme() == Office2016DarkGray)
                  optTitle.clrText = !active ? d.m_clrFrameCaptionTextInActive : d.m_clrFrameCaptionTextActive;
                    ...
                    if (!soImage.isNull())
                    {
                        ...
                        if (rcSrc.height() > 0)
                        {
                            QRect rcArea(QPoint(rcRibbonClient.left(), rcRibbonTabs.top() - rcSrc.height()), 
                                QPoint(rcRibbonClient.right(), rcRibbonTabs.top() - 1));
            ...
            if (optTitleBar->quickAccessOnTop && optTitleBar->quickAccessVisible && optTitleBar->quickAccessVisibleCount > 0)
            {
                if (optTitleBar->existSysButton && !optTitleBar->normalSysButton)
                {
                    ...
                    QRect rcQuickAccessArea = rcQuickAccess.adjusted(-16, -1, 4, 0);
            ...
            if ((themeId < Office2013White || themeId < Office2016DarkGray || !optTitleBar->isBackstageVisible) && optTitleBar->listContextHeaders && optTitleBar->listContextHeaders->count() > 0)
                proxy()->drawPrimitive((QStyle::PrimitiveElement)RibbonStyle::PE_RibbonContextHeaders, opt, p, w);

            if (!dwmEnabled)
            {
                p->save();
                if (themeId == Office2016Colorful || themeId == Office2016DarkGray)
                {
                    if (optTitleBar->isBackstageVisible)
                    {
                        if (themeId == Office2016Colorful)
                            p->setPen(QColor(119, 119, 119).darker());
                        else
                            p->setPen(active ? QColor(255, 255, 255) : QColor(229, 229, 229));
                    }
                    else
                    ...
                    p->setPen(!active ? d.m_clrFrameCaptionTextInActive : d.m_clrFrameCaptionTextActive);
                p->setFont( w ? w->font() : QApplication::font("QMenuBar"));
                ...
                Qt::Alignment fl = Qt::AlignCenter;//optTitleBar->quickAccessVisible ? Qt::AlignCenter : Qt::AlignLeft | Qt::AlignVCenter;
                ...
                if (!dwmEnabled && optTitleBar->subControls & SC_TitleBarSysMenu && optTitleBar->titleBarFlags & Qt::WindowSystemMenuHint) 
                {
                    ...
                    if (!optTitleBar->icon.isNull())
                    {
                        ...
                        if (HICON hIcon = optTitleBar->frameHelper->windowIcon(optTitleBar->icon))
                        {
                            ...
                            if (!px.isNull())
                            {
                                int iconSize = optTitleBar->frameHelper->iconSize();//px.size();
                                int controlHeight = optTitleBar->rect.height();
                                int hPad = (controlHeight - iconSize)/2;
                                int vPad = (controlHeight - iconSize)/2;
                                int br = optTitleBar->frameHelper->frameBorder();
                                QRect r = QRect(hPad - br, vPad - br, iconSize, iconSize);
    ...
    if (qobject_cast<const RibbonPageSystemPopup*>(widget))
    {
        if (const QStyleOptionMenuItem* menuitem = qstyleoption_cast<const QStyleOptionMenuItem*>(opt)) 
        {
            if (menuitem->text.count(QString(_qtn_PopupLable)) > 0)
            {
                ...
                {
                    p->fillRect(menuitem->rect.adjusted(-1, 0, 1, 0), d.m_clrControlGalleryLabel);
                }
                else
                {
                ...
                text = p->fontMetrics().elidedText(text.remove(QString(_qtn_PopupLable)), Qt::ElideRight, rcText.adjusted(2, 0, -2, 0).width());
            ...
            else
            {
                ...
                const int nIconSize = proxy()->pixelMetric(PM_SmallIconSize, opt, widget); //pixelMetric(PM_ToolBarIconSize, opt, widget);
        ...
        else
        {
            ...
            if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly)
            {
                ...
                if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
                {
                    ...
                    if (!hasArrow)
                        drawItemPixmap(p, pr, Qt::AlignTop | Qt::AlignHCenter, pm);
                }
                else
                {
                    QRect pmRect(QPoint(4, (rect.height() - pmSize.height())/2), pmSize);
                    if (!hasArrow) 
                        QStyle::drawItemPixmap(p, QStyle::visualRect(opt->direction, rect, pmRect), Qt::AlignCenter, pm);
                    ...
                    proxy()->drawItemText(p, tcText, alignment, toolbutton->palette, /*toolbutton->state & State_Enabled*/true, toolbutton->text, QPalette::ButtonText);
                }
            }
            else
            {
                ...
                QStyle::drawItemPixmap(p, pr, Qt::AlignCenter, pm);
            }
        }
    }
    return true;
...
bool RibbonStyle::drawToolButton(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        if (toolbutton->subControls & SC_ToolButtonMenu) 
        {
            ...
            if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
            {
                bool twoLines = canWordWrap(toolbutton->rect, toolbutton->fontMetrics, toolbutton->text);
                rcArrow = QRect(QPoint(!twoLines ? opt->rect.width()/2 - 2 : opt->rect.right()-7, opt->rect.bottom()-8), QSize(5, 4));
            }
            else
...
bool RibbonStyle::drawGroupControlEntry(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const RibbonStatusBarSwitchGroup* switchGroup = qobject_cast<const RibbonStatusBarSwitchGroup*>(widget->parentWidget()))
    {
        int index = widget->property(_qtn_Index_Group).toInt();
        int count = switchGroup->property(_qtn_Switch_Count).toInt();
    ...
    if (widget->property(_qtn_Group).toBool())
    {
        if (const QStyleOptionToolButton*toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
        {
            if (toolbutton->subControls & SC_ToolButtonMenu)
